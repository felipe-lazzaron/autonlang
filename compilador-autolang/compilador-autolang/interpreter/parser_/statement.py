class StatementParser:
    ...
    def parse_expression(tokenizer, symbol_table):
        ...
        while tokenizer.next.type in {'OPERATOR'} and tokenizer.next.value in {'+', '-', '*', '/'}:
            op = tokenizer.next
            tokenizer.select_next()
    ...
    def parse_factor(tokenizer, symbol_table):
        if tokenizer.next.type == 'NUMBER':
            ...
            raise SyntaxError(f"Unexpected token: {tokenizer.next.type} {tokenizer.next.value}")

    @staticmethod
    def parse_condition(tokenizer, symbol_table):
        left = StatementParser.parse_expression(tokenizer, symbol_table)
        if tokenizer.next.type != 'OPERATOR' or tokenizer.next.value not in {'==', '!=', '<', '<=', '>', '>='}:
